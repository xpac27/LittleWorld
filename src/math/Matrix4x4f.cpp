...
void Matrix4x4f::translate(const Vector3 &trans)
{
    identity();
...
void Matrix4x4f::rotate(const float &angle, Vector3 &axis)
{
    float s = sin(DEGTORAD(angle));
    float c = cos(DEGTORAD(angle));

    Vector3Util::Normalize(axis);
...
void Matrix4x4f::scale(const Vector3 &scale)
{
    identity();
...
void Matrix4x4f::transformPoint(Vector3 *vec)
{
    Vector3 &v = *vec;
...
void Matrix4x4f::transformVector(Vector3 *vec)
{
    Vector3 &v = *vec;
